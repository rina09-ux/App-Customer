...
import { getCoreCloudAccounts, getCoreCloudAssets, startCoreCloudScan, CoreCloudAccount, CoreCloudAsset } from '../lib/assetsCloudApi';
...
export const CoreAssetsCloudView: React.FC<CoreAssetsCloudViewProps> = ({ showToast = (_message: string) => {} }) => {
  ...
  const [assets, setAssets] = useState<CoreCloudAsset[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [nextAccounts, nextAssets] = await Promise.all([
        getCoreCloudAccounts(),
        getCoreCloudAssets({ provider, search }),
      ]);
  ...
  const providerOptions = useMemo(() => {
    const values = new Set(accounts.map((x) => x.provider));
  ...
  const handleScan = async (account: CoreCloudAccount) => {
    ...
    try {
      await startCoreCloudScan(numericId);
      showToast(`Scan ${providerLabel(account.provider)} untuk ${account.name} masuk antrean di Core.`);
      await load();
    } catch (err) {
      showToast(err instanceof Error ? err.message : 'Gagal memulai scan.');
    ...
    <div id="assets-cloud-view" className="flex-1 overflow-y-auto bg-[#f8fafc] dark:bg-slate-950 px-4 sm:px-8 py-6 max-w-7xl mx-auto w-full space-y-7">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[11px] font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 font-mono">NusaSec Secure Data Plane</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Multi-Cloud Connections & Asset Inventory</h1>
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Data tenant langsung dari NusaSec-Core. Tidak menggunakan mock sebagai sumber produksi.</p>
        </div>
        <button onClick={() => showToast('Tambah koneksi cloud dilakukan melalui Core control plane dan identity binding.')} className="px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs sm:text-sm font-semibold flex items-center gap-2 shadow-xs">
          <Plus className="w-4 h-4" /> Tambah Koneksi Cloud
        </button>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {accounts.map((account) => (
            <div key={account.id} className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl p-5 shadow-2xs space-y-4">
              ...
              <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between"><span className="text-[11px] text-slate-500">{account.lastValidatedAt ? new Date(account.lastValidatedAt).toLocaleString() : 'Belum discan'}</span><button onClick={() => void handleScan(account)} disabled={scanningId === Number(account.id)} className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-900 dark:bg-blue-600 text-white text-[11px] font-semibold disabled:opacity-50"><ScanLine className="w-3.5 h-3.5" /> {scanningId === Number(account.id) ? 'Queue…' : 'Scan'}</button></div>
            </div>
          ))}
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-2xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div><h3 className="font-bold text-slate-900 dark:text-white text-base">Katalog Seluruh Aset Infrastruktur</h3><p className="text-xs text-slate-500 dark:text-slate-400">Filter dan pencarian dieksekusi oleh Core dengan tenant isolation.</p></div>
          <div className="flex flex-wrap items-center gap-2">
            <select value={provider} onChange={(e) => setProvider(e.target.value)} className="px-3 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-200 max-w-full min-w-0"><option value="all">Semua Provider</option>{providerOptions.filter((x) => x !== 'all').map((x) => <option key={x} value={x}>{providerLabel(x)}</option>)}</select>
            <div className="relative min-w-0 w-full sm:w-60"><Search className="w-3.5 h-3.5 text-slate-400 absolute left-3 top-2.5" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari nama aset / ARN..." className="w-full pl-8 pr-3 py-2 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white min-w-0" /></div>
          </div>
        </div>
