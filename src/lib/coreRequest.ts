...
const CORE_API_URL = (configuredCoreUrl || '').replace(/\/$/, '');
const CORE_REQUEST_TIMEOUT_MS = 20000;

if (!CORE_API_URL) {
  throw new Error('NusaSec Core URL is required in all builds (set VITE_NUSASEC_CORE_API_URL or VITE_NUSASEC_CORE_URL).');
}

let csrfToken: string | null = null;

export function setCoreCsrfToken(token: string | null): void { csrfToken = token; }
export function getCoreCsrfToken(): string | null { return csrfToken; }
...
async function loadCsrfToken(): Promise<string> {
  ...
  const response = await fetch(`${CORE_API_URL}/api/v1/auth/csrf`, { credentials: 'include', headers: { Accept: 'application/json' }, cache: 'no-store', signal: timeoutSignal() });
  if (!response.ok) throw new Error(`Unable to obtain Core CSRF token (${response.status})`);
  const body = await response.json().catch(() => ({}));
  if (typeof body?.csrf_token !== 'string' || !body.csrf_token) throw new Error('Core did not return a valid CSRF token');
...
export async function requestCore<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  try {
    response = await fetch(`${CORE_API_URL}${path}`, { ...init, credentials: 'include', headers, cache: 'no-store', signal: timeoutSignal(init.signal) });
  } catch (error) {
    if (error instanceof DOMException && error.name === 'AbortError') throw new Error('NusaSec-Core request timed out or was cancelled');
    throw error;
  }
  const body = await response.json().catch(() => ({}));
...
export async function requestCoreMultipart<T>(path: string, body: FormData, signal?: AbortSignal): Promise<T> {
  ...
  const response = await fetch(`${CORE_API_URL}${path}`, { method: 'POST', credentials: 'include', headers: new Headers({ Accept: 'application/json', 'X-CSRF-Token': token }), body, cache: 'no-store', signal: timeoutSignal(signal) });
